if __name__ == '__main__':
    ...
    class testMemory(unittest.TestCase):
        ...
        def testDynamicMemoryCanBeWrittenAndReadFrom(self):
            self.assertEqual(0x00, self.memory.loadb(0x42))
            self.memory.storeb(0x42, 0x20)
            self.assertEqual(0x20, self.memory.loadb(0x42))
            self.assertEqual(0x2000, self.memory.loadw(0x21))
            self.memory.storew(0x21, 0x1234)
            self.assertEqual(0x1234, self.memory.loadw(0x21))
            self.assertEqual(0x12, self.memory.loadb(0x42))
            self.assertEqual(0x34, self.memory.loadb(0x43))
        
        # Spec 1.1.2
        def testStaticMemoryCanBeReadFromOnly(self):
            self.assertEqual(0x00, self.memory.loadb(0x50))
            self.assertEqual(0x01, self.memory.loadb(0x51))
            self.assertEqual(0x20, self.memory.loadb(0x52))
            self.assertEqual(0x2000, self.memory.loadw(0x29))
            self.assertRaises(memory.ReadOnlyException, lambda m: m[0].storeb(m[1], m[2]), (self.memory, 0x51, 0x00))
            self.assertRaises(memory.ReadOnlyException, lambda m: m[0].storew(m[1], m[2]), (self.memory, 0x29, 0x00))
            
        # Spec 1.1.3
        def testHighMemoryCannotBeAccessedDirectly(self):
            self.assertRaises(memory.OutOfRangeException, lambda m: m[0].storeb(m[1], m[2]), (self.memory, 0x61, 0x00))
            self.assertRaises(memory.OutOfRangeException, lambda m: m[0].storew(m[1], m[2]), (self.memory, 0x31, 0x00))
            self.assertRaises(memory.OutOfRangeException, lambda m: m[0].loadb(m[1]), (self.memory, 0x61))
            self.assertRaises(memory.OutOfRangeException, lambda m: m[0].loadw(m[1]), (self.memory, 0x31))

        # Spec 1.2.1
        def testByteAndWordAddressesWork(self):
            self.assertEquals(0x00, self.memory.loadb(0))
            self.assertEquals(0x01, self.memory.loadb(1))
            self.assertEquals(0x0001, self.memory.loadw(0))
            self.assertEquals(0x0203, self.memory.loadw(1))
            self.assertEquals(0x02, self.memory.loadb(0x22))
            self.assertEquals(0x03, self.memory.loadb(0x23))
            self.assertEquals(0x0203, self.memory.loadw(0x11))
